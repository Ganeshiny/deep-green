...
def correct_residue(x, target):
    try:
        sl = seq1(x.resname, undef_code='-', custom_map=protein_letters_3to1 )
        if sl == target:
            return True
        return False
    except KeyError:
...
class DistanceMapBuilder:
        ...
        for chain_name in structure_container.chains:
            ...
            if chain['seqres-seq'] is not None and len(chain['seqres-seq']) > 0:
                ...
                for item in pattern:
                    if len(item) > 0:
                        if item[0] == 't':
                            target += str(item[20:].split(' ')[0])
                        elif item[0] == 'q':
                            query += str(item[20:].split(' ')[0])
                        else:
                            mask += str(item[20:].split(' ')[0])

                ## TESTCODE
                # aligned_seqres_seq, mask, aligned_atom_seq = pattern[:3]
                aligned_seqres_seq = target
                mask = mask
                ...
                #print(f"residues: {residues}, re-indexed residues: {reindexed_residues}, final-residue-list: {final_residue_list}")
                ...
                for i in range(len(aligned_atom_seq)):
                    ...
                    if current_aligned_atom_residue_letter != '-' and mask[i] == '|':
                        ...
                        #print(f"current_aligned_atom_residue_letter: {current_aligned_atom_residue_letter}, mask: {mask[i]}")
                        ...
                        #print(f"final-residue-list: {final_residue_list}")
                    else:
    ...
    def __calc_dist_matrix(self, chain_one):
        ...
        answer = np.zeros((len(chain_one), len(chain_one)), dtype = float)
